...
class BackupManager(private val sling: SlingExtension) {
     ...
     */
    val uploadUrl = sling.obj.string {
        sling.prop.string("localInstance.backup.uploadUrl")?.let { set(it) }
    }

    /**
     * URL to remote backup file.
     */
    val downloadUrl = sling.obj.string {
        sling.prop.string("localInstance.backup.downloadUrl")?.let { set(it) }
    }
     ...
     */
    val localDir = sling.obj.dir {
        convention(sling.project.layout.projectDirectory.dir("$OUTPUT_DIR/${BackupType.LOCAL.dirName}"))
        sling.prop.file("localInstance.backup.localDir")?.let { set(it) }
    }
     ...
     */
    val remoteDir = sling.obj.dir {
        convention(sling.project.layout.projectDirectory.dir("$OUTPUT_DIR/${BackupType.REMOTE.dirName}"))
        sling.prop.file("localInstance.backup.remoteDir")?.let { set(it) }
    }
     ...
     */
    val suffix = sling.obj.string {
        convention(SUFFIX_DEFAULT)
        sling.prop.string("localInstance.backup.suffix")?.let { set(it) }
    }
    ...
    private var namer: () -> String = {
        val parts = mutableListOf<String>().apply {
            add(sling.project.rootProject.name)
            add(Formats.dateFileName())
            val version = sling.project.version.toString()
    ...
    private var selector: Collection<BackupSource>.() -> BackupSource? = {
        val name = sling.prop.string("localInstance.backup.name") ?: ""
        when {
    ...
    companion object {
        const val OUTPUT_DIR = ".gradle/sling/localInstance/backup"
