...
import com.cognifide.gradle.sling.base.vlt.SyncTask
...
class PropertyParser(val project: Project) {
    ...
    companion object {
        ...
        const val FORCE_PROP = "sling.force"
    ...
    val configProps: Map<String, Any>
        get() = mapOf("config" to SlingConfig.of(project))
    ...
    fun checkForce() {
        if (!isForce()) {
            throw SlingException("Unable to perform unsafe operation without param '-P$FORCE_PROP'")
        }
    }
