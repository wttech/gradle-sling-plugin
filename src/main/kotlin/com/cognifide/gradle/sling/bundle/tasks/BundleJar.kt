...
import com.cognifide.gradle.sling.bundle.BundleException
import com.cognifide.gradle.sling.common.instance.service.osgi.Bundle
import com.cognifide.gradle.sling.common.utils.normalizeSeparators
...
open class BundleJar(private val jar: Jar) {
    ...
    private val sling = project.sling
    ...
    @Input
    @Optional
    val installRunMode = sling.obj.string()
     ...
     */
    @Input
    val vaultFilter = sling.obj.boolean {
        convention(true)
        sling.prop.boolean("bundle.vaultFilter")?.let { set(it) }
    }
     ...
     */
    @Input
    val attributesConvention = sling.obj.boolean {
        convention(true)
        sling.prop.boolean("bundle.attributesConvention")?.let { set(it) }
    }
     ...
     */
    @Input
    val javaPackage = sling.obj.string {
        convention(sling.obj.provider {
            val group = sling.project.group.toString()
            val name = sling.project.name
     ...
     */
    @Input
    val javaPackageOptions = sling.obj.string {
        convention("-split-package:=merge-first")
        sling.prop.string("bundle.javaPackageOptions")?.let { set(it) }
    }

    @Internal
    val importPackages = sling.obj.strings { convention(listOf()) }

    @Input
    val importPackageSuffix = sling.obj.string { convention("*") }

    @Internal
    val exportPackages = sling.obj.strings { convention(listOf()) }

    @Internal
    val privatePackages = sling.obj.strings { convention(listOf()) }
    ...
    private fun applyArchiveDefaults() {
        jar.archiveBaseName.set(sling.commonOptions.baseName)
    }

    private fun applyBndToolDefaults() {
        val instructionFile = File("${sling.project.file("bnd.bnd")}")
    ...
    fun runBndTool() {
        try {
            ...
            sling.logger.error("BND tool error: https://bnd.bndtools.org", ExceptionUtils.getRootCause(e))
    ...
    private fun applyAttributesConvention() {
        ...
        if (!hasAttribute(Bundle.ATTRIBUTE_NAME) && !sling.project.description.isNullOrBlank()) {
            displayName = sling.project.description
        }
    ...
    @Input
    val activatorClasses = sling.obj.strings {
        convention(listOf("Activator", "BundleActivator"))
    }

    @Input
    val activatorSourceSets = sling.obj.map<String, String> {
        convention(mapOf(
                "java" to "java",
                "kotlin" to "kt",
                "scala" to "scala"
    ...
    private fun findActivator(pkg: String): String? {
        for ((sourceSet, ext) in activatorSourceSets.get()) {
            for (activatorClass in activatorClasses.get()) {
                if (sling.project.file("src/main/$sourceSet/${pkg.replace(".", "/")}/$activatorClass.$ext").exists()) {
