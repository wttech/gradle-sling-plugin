...
open class PackageConfig : SlingDefaultTask() {
     ...
     */
    @Internal
    val saveDir = sling.obj.dir { convention(sling.packageOptions.configDir) }

    /**
     * Unique ID of OSGi config to be saved.
     */
    @Internal
    val pid = sling.obj.string { convention(sling.prop.string("package.config.pid")) }
    ...
    fun sync() {
        ...
        instance.get().sync {
            common.progress {
                ...
                val configPidPattern = pid.orNull ?: listOfNotNull(sling.javaPackage).ifEmpty { sling.javaPackages }.joinToString(",") { "$it.*" }
