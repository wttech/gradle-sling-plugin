...
import com.cognifide.gradle.sling.common.CommonOptions
...
import com.cognifide.gradle.sling.common.pkg.PackageOptions
...
import com.cognifide.gradle.sling.common.pkg.vault.VaultSummary
import com.cognifide.gradle.sling.common.utils.ProcessKiller
import com.cognifide.gradle.sling.common.utils.WebBrowser
import com.cognifide.gradle.sling.pkg.PackageSyncPlugin
...
class SlingExtension(val project: Project) : Serializable {
    ...
    fun namedInstance(desiredName: String? = prop.string("instance.name"), defaultName: String = "${commonOptions.env.get()}-*"): Instance {
        return findInstance(desiredName, defaultName)
                ?: throw SlingException("Instance named '${desiredName ?: defaultName}' is not defined.")
    }
    ...
    fun syncFiles(instances: Iterable<Instance>, packages: Iterable<File>, action: InstanceSync.(File) -> Unit) {
        packages.forEach { pkg -> // single Sling instance dislikes parallel package / OSGi bundle installation
            common.parallel.with(instances) { // but same file could be in parallel deployed on different Sling instances
    ...
    companion object {
        ...
        fun of(project: Project): SlingExtension {
            return project.extensions.findByType(SlingExtension::class.java)
                    ?: throw SlingException("${project.displayName.capitalize()} must have at least one of following plugins applied: $PLUGIN_IDS")
        }
    }
}

val Project.sling get() = SlingExtension.of(this)
