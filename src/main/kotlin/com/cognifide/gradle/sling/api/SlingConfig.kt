...
class SlingConfig(
     ...
     */
    @Input
    val environment: String = props.string("sling.env", {
        System.getenv("SLING_ENV") ?: "local"
    })

    /**
     * List of Sling instances on which packages could be deployed.
     ...
     */
    @Input
    var instanceConnectionTimeout: Int = props.int("sling.instance.connectionTimeout", 5000)
     ...
     * - generated Sling specific manifest instructions like 'Sling-Model-Packages'.
     * - performed additional component stability checks during 'slingAwait'
     */
    @Input
     ...
     * like 'Bundle-SymbolicName', 'Export-Package' or Sling specific like 'Sling-Model-Packages'
     ...
     */
    @Internal
    var packageBuildDate: Date = props.date("sling.package.buildDate", Date())
     ...
     */
    @Input
    var uploadRetryTimes: Int = props.int("sling.upload.retry.times", 3)
     ...
     */
    @Input
    var installRetryTimes: Int = props.int("sling.install.retry.times", 1)
     ...
     */
    @Input
    var vaultLineSeparator: String = props.string("sling.vlt.lineSeparator", "LF")

    /**
     * Path in which local Sling instances will be stored.
     *
     * Default: "${System.getProperty("user.home")}/.sling/${project.rootProject.name}"
     */
    @Input
    var createPath: String = "${System.getProperty("user.home")}/.sling/${project.rootProject.name}"
     ...
     */
    @Input
    var awaitFast: Boolean = props.flag("sling.await.fast")
     ...
     */
    @Input
    var awaitFastDelay: Long = props.long("sling.await.fast.delay", TimeUnit.SECONDS.toMillis(1))

    /**
     * Do not fail build but log warning when there is still some unstable or unhealthy instance.
     */
    @Input
    var awaitResume: Boolean = props.flag("sling.await.resume")
     ...
     */
    @Input
    var awaitStableInterval: Long = props.long("sling.await.stable.interval", TimeUnit.SECONDS.toMillis(1))
     ...
     */
    @Input
    var awaitStableTimes: Long = props.long("sling.await.stable.times", 60 * 5)
     ...
     */
    @Input
    var awaitStableAssurances: Long = props.long("sling.await.stable.assurances", 3L)
     ...
     */
    @Input
    var satisfyGroupName = props.string("sling.satisfy.group.name", "*")

    /**
     * Determines a Vault filter used to checkout JCR content from running Sling instance.
     *
     ...
     */
    @Input
    var checkoutFilterPath: String = props.string("sling.checkout.filterPath", "")
    ...
    /**
     * Define here properties that will be skipped when pulling JCR content from Sling instance.
     *
     ...
     */
    @Input
    var debugPackageDeployed: Boolean = props.boolean("sling.debug.packageDeployed", !project.gradle.startParameter.isOffline)

    /**
     * Turn on/off default system notifications.
     */
    @Internal
    var notificationEnabled: Boolean = props.flag("sling.notification.enabled")
    ...
    /**
     * Declare new deployment target (Sling instance).
     */
    ...
    fun localInstance(httpUrl: String, configurer: LocalInstance.() -> Unit) {
        instance(LocalInstance.create(httpUrl, {
            this.environment = this@SlingConfig.environment
    ...
    private fun instance(instance: Instance) {
        if (instances.containsKey(instance.name)) {
            throw SlingException("Instance named '${instance.name}' is already defined. Enumerate instance types (for instance 'author1', 'author2') or distinguish environments.")
        }
    ...
    fun validate() {
        if (bundlePath.isBlank()) {
            throw SlingException("Bundle path cannot be blank")
        }

        if (contentPath.isBlank()) {
            throw SlingException("Content path cannot be blank")
        }
    ...
    companion object {
         ...
         */
        fun of(project: Project): SlingConfig {
            val extension = project.extensions.findByType(SlingExtension::class.java)
                    ?: throw SlingException(project.toString().capitalize()
                            + " has neither '${PackagePlugin.ID}' nor '${InstancePlugin.ID}' plugin applied.")

            return extension.config
        }

        fun of(task: DefaultTask): SlingConfig {
            return of(task.project)
        }

        fun pkg(project: Project): ComposeTask {
            val task = project.tasks.findByName(ComposeTask.NAME)
                    ?: throw SlingException("${project.toString().capitalize()} has no task named"
