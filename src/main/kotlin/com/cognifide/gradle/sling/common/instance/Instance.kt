...
open class Instance(@Transient @get:JsonIgnore protected val sling: SlingExtension) : Serializable {
    ...
    @get:JsonIgnore
    val sync get() = InstanceSync(sling, this)
    ...
    fun checkState(options: CheckRunner.() -> Unit = {}) = CheckRunner(sling).apply {
    ...
    fun validate() {
        if (httpUrl.isBlank()) {
            throw SlingException("HTTP URL cannot be blank in $this")
        }
        ...
        if (id.isBlank()) {
            throw SlingException("ID cannot be blank in $this")
        }
    }

    companion object {

        fun create(sling: SlingExtension, httpUrl: String, configurer: Instance.() -> Unit = {}): Instance {
            return Instance(sling).apply {
        ...
        fun properties(sling: SlingExtension): List<Instance> {
            return sling.project.properties.filterKeys {
                Patterns.wildcard(it, "instance.*.httpUrl")
            }.keys.mapNotNull { property ->
                val name = property.split(".")[1]
                val nameParts = name.split("-")
                if (nameParts.size != 2) {
                    sling.logger.warn("Instance name has invalid format '$name' in property '$property'.")
                    return@mapNotNull null
                }

                val props = sling.project.properties.filterKeys {
                ...
                if (props["httpUrl"].isNullOrBlank()) {
                    sling.logger.warn("Instance named '$name' must have property 'httpUrl' defined.")
                    return@mapNotNull null
                }
                ...
                when (type) {
                    PhysicalType.LOCAL -> LocalInstance.create(sling, httpUrl) {
                        this.env = env
                        this.id = id
                    ...
                    PhysicalType.REMOTE -> create(sling, httpUrl) {
                        this.env = env
                        this.id = id
